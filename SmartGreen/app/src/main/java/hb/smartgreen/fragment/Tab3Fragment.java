...
/**
 * A simple {@link Fragment} subclas3s.
 */
@ContentView(R.layout.fragment_tab3)
public class Tab3Fragment extends BaseFragment implements StationListView {
    @ViewInject(R.id.pull_to_refresh)
    private PullToRefreshView mPullToRefreshView;

    public static final int REFRESH_DELAY = 2000;

    @ViewInject(R.id.focusStationItem)
    ...
    public void refreshListData(StationSource stationSource){
        stationList.setAdapter(new hb.smartgreen.adapter.StationAdapter(stationSource,this.getContext()));
        stationList.getParent().requestDisallowInterceptTouchEvent(true);
    }

    @Override
    public void onViewCreated(View view, Bundle savedInstanceState)  {
        realTimeStationList.loadListData("request tag", 3, "eventtag", 1, true);
