$(document).ready(function () {
    ...
    function doReady() {
        $(".NewsOperation").on("click", ".addNewsBtn", function () {
            var id = '00000000-0000-0000-0000-000000000000';
            showDialog(1000, 680, "http://" + window.location.host + "/News/NewsManage/" + id, function () { GetList(1) });
        }).on("click", ".delNewsBtn", function () {
    ...
    function DeleteList(allId) {
        ...
        var postdata = { cmd: "deleteNews", id: allId };
    ...
    function Operation() {
        $(".tbody_newsList").on("click", ".newsShow", function () {
            ...
            window.open("http://" + window.location.host + "/News/NewsShow/" + id);
        }).on("click", ".newsEdit", function () {
            var id = $(this).parent().parent().find(".chk").attr("id");
            showDialog(1000, 680, "http://" + window.location.host + "/News/NewsManage/" + id);
        });
    }
    ...
    function BindChkAll() {
        $(document).on("click", ".chk-all", function () {
            ...
            $(".NewsList").find("tbody :checkbox").each(function () {
                this.checked = checked;
            });
        })
        $(".NewsList").on("click", ".chk", function () {
            $(".chk-all").removeAttr("checked");
        });
    }
});