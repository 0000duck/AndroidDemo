...
{
    public class News :IDAL.INews
    {
        public static SqlParameter[] GetModelPare(Model.News model)
        {
        ...
        public static readonly string updatesql = "update News set Title=@Title,Content=@Content,Type=@Type,Author=@Author,CreateTime=@CreateTime,ViewNum=@ViewNum,Editor=@Editor,EditTime=@EditTime,Attachment=@Attachment where NewsID=@NewsID";
        public static readonly string insertsql = "insert into News(NewsID,Title,Content,Type,Author,CreateTime,ViewNum,Editor,EditTime,Attachment) values (@NewsID,@Title,@Content,@Type,@Author,@CreateTime,@ViewNum,@Editor,@EditTime,@Attachment)";
        public bool Add(Model.News model)
        {
        ...
        public bool DeleteListByID(List<Guid> idList)
        {
            ...
            strSql.Append("delete from News where NewsID in (");
        ...
        private static T[] DateTableToArray<T>(DataTable dt) where T : Model.News, new()
        {
        ...
        private static T DataRowToModel<T>(DataRow Rows) where T : Model.News, new()
        {
            var model = new T();
            model.NewsID = (Guid)Rows["NewsID"];
