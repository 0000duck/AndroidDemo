...
namespace NetSchool.DAL
{
    public class Law:IDAL.ILaw
    {
        ...
        public static readonly string updatesql = "update Law set LawID=@LawID,Title=@Title,Content=@Content,Type=@Type,Author=@Author,CreateTime=@CreateTime,ViewNum=@ViewNum,Editor=@Editor,EditTime=@EditTime,Attachment=@Attachment where LawID=@LawID";
        public static readonly string insertsql = "insert into Law(LawID,Title,Content,Type,Author,CreateTime,ViewNum,Editor,EditTime,Attachment) values (@LawID,@Title,@Content,@Type,@Author,@CreateTime,@ViewNum,@Editor,@EditTime,@Attachment)";
        ...
        public bool DeleteListByID(List<Guid> idList)
        {
            ...
            strSql.Append("delete from Law where LawID in (");
        ...
        public T[] SearchList<T>(Guid? ID, bool IsSearchTitle, string Title, bool IsSearchContent, string Content, int? Type, bool IsSearchAuthor, string Author, DateTime? StartTime, DateTime? EndTime, Common.Info.PageInfo pageInfo) where T : Model.Law, new()
        {
            StringBuilder strSql = new StringBuilder();
            var parameters = new List<SqlParameter>();
            strSql.Append("select [LawID],[Title],[Content],[Type],[Author],[CreateTime],[ViewNum],[Editor],[EditTime],[Attachment] from Law where 1=1");
            if (ID.HasValue)
            {
                strSql.Append(" and LawID=@LawID");
                var para = new SqlParameter("@LawID", SqlDbType.UniqueIdentifier, 16);
                para.Value = ID;
                parameters.Add(para);
            }
            if (IsSearchTitle)
        ...
        private static T[] DateTableToArray<T>(DataTable dt) where T : Model.Law, new()
        {
        ...
        private static T DataRowToModel<T>(DataRow Rows) where T : Model.Law, new()
        {
            var model = new T();
            model.LawID = (Guid)Rows["LawID"];
