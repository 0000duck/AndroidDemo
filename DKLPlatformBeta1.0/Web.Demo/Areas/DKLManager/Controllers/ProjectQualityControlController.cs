...
namespace Web.Demo.Areas.DKLManager.Controllers
{
    public class ProjectQualityControlController : AdminControllerBase
    {
        ...
        public ActionResult WaterEdit(int id)
        {
            var model = new ProjectWholeInfoViewModel();
            model.projectBasicinfo = this.IDKLManagerService.GetProjectInfo(id);
            if (model.projectBasicinfo != null)
        ...
        public ActionResult Submit(int id, string ProjectCategory)
        {
            ...
            if (ProjectCategory == "2?" || ProjectCategory == "1?")
            {
                var users = this.AccountService.GetUserListV(13).Select(c => new { Id = c.ID, Name = c.Name });
            ...
            {
                var users = this.AccountService.GetUserListG(5).Select(c => new { Id = c.ID, Name = c.Name });
                ViewData.Add("PersonW", new SelectList(users, "Name", "Name"));
                var model = new ProjectWholeInfoViewModel();
                model.projectBasicinfo = this.IDKLManagerService.GetProjectInfo(id);
                if (model.projectBasicinfo != null)
                {
        ...
        public ActionResult Submit(int id, FormCollection collection)
        {
            ...
            var model = new ProjectWholeInfoViewModel();
            model.projectBasicinfo = this.IDKLManagerService.GetProjectInfo(id);
            if (model.projectBasicinfo.ProjectCategory == 2 || model.projectBasicinfo.ProjectCategory == 1)
            {
            ...
            else if (model.projectBasicinfo.ProjectCategory == 5)
            {
                var user = this.AccountService.GetUser(this.LoginInfo.LoginName);
                var nn = this.IDKLManagerService.SelectContractInfo(model.projectBasicinfo.ProjectName);
                model.projectBasicinfo.SignTime = nn.ContractDate;
                ...
                models.Instructions = user.LoginName + "提交";
                this.IDKLManagerService.InsertTimeInstructions(models);
                model.projectBasicinfo.ProjectStatus = (int)EnumProjectSatus.WaterS;
                model.projectBasicinfo.Remarks = "";
                model.projectBasicinfo.ProjectPersonCategory = 0;
                model.projectBasicinfo.ProjectProblem = 0;
                model.projectBasicinfo.ProjectProblemDescribe = "";
        ...
        public ActionResult WaterSubmit(int id)
        {
            var userW = this.AccountService.GetUserList(9).Select(c => new { Id = c.ID, Name = c.Name });
            ViewData.Add("PersonW", new SelectList(userW, "Name", "Name"));
            var model = new ProjectWholeInfoViewModel();
            model.projectBasicinfo = this.IDKLManagerService.GetProjectInfo(id);
            if (model.projectBasicinfo != null)
            {
        ...
        [HttpPost]
        public ActionResult WaterSubmit(int id, FormCollection collection)
        {
            var model = new ProjectWholeInfoViewModel();
            ...
            if (model.projectBasicinfo != null)
            {
                ...
                model.projectBasicinfo.ProjectProblem = 0;
                model.projectBasicinfo.ProjectProblemDescribe = "";
                model.projectBasicinfo.Person = collection["PersonW"];
                this.IDKLManagerService.UpdateProjectInfo(model.projectBasicinfo);
            }
            return this.RefreshParent();
        }
