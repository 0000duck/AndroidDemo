...
namespace Web.Demo.Common
{
    public abstract class AdminControllerBase : Web.Common.ControllerBase
    {
        ...
        protected void UpdateViewModels(FormCollection collection, ref DeviceModel model)
        {
            model.DeivceName = collection["DeivceName"];
            model.Number = collection["Number"];
            model.ModelNum = collection["ModelNum"];
            model.Manufacturer = collection["Manufacturer"];
            model.SkillParameter = collection["SkillParameter"];
            model.CalibrationPeriod = collection["CalibrationPeriod"];
            model.CalibrationTime = Convert.ToDateTime(collection["CalibrationTime"]);
            model.BuyTime = Convert.ToDateTime(collection["BuyTime"]);
            string strCategory = (collection["DeviceMold"] == null) ? collection["DeviceMold"] : collection["DeviceMold"];
            int category;
            if (int.TryParse(strCategory, out category))
            {
                model.DeviceMold = category;
            }
            string strCategoryq = (collection["DeviceIdentityid"] == null) ? collection["DeviceIdentityid"] : collection["DeviceIdentityid"];
            int categoryq;
            if (int.TryParse(strCategoryq, out categoryq))
            {
                model.DeviceIdentityid = categoryq;
            }
        }
        protected void UpdateViewModels(FormCollection collection, ref ProjectInfo model)
        {
            model.BillNumBer = collection["BillNumBer"];
            model.Money = collection["Money"];
             model.NoMoney = collection["NoMoney"];
            model.MakeOutAnInvoiceTime = Convert.ToDateTime(collection["MakeOutAnInvoiceTime"]);
        }
        ...
        protected void RenderMyViewDatas(ProjectWholeInfoViewModel model)
        {
            ...
            ViewData.Add("ProjectStatus", EnumHelper.GetEnumTitle((EnumProjectSatus)model.projectBasicinfo.ProjectStatus));
            ViewData.Add("ProjectPersonCategory", EnumHelper.GetEnumTitle((EnumProjectPersonCategory)model.projectBasicinfo.ProjectPersonCategory));
            if ((model.projectBasicImgFile != null) && (!string.IsNullOrEmpty(model.projectBasicImgFile.FilePath)))
            {
                List<string> htmlFIles = new List<string>();
                model.projectBasicImgFile.FilePath.Split(',').ToList().ForEach(f =>
        ...
        protected override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            else
            {
                ...
                if (user.SecondStatus == true&&user.SecondAccountType >=0)
                {
