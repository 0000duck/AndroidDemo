...
namespace OfficeDocGenerate
{
    public class CreateAnalysis
    {
        ...
        public CreateAnalysis(Costing model)
        {
            this.model = model;
        }
        ...
        {
            string fileName = "";
            string name = "评价项目成本分析表" + DateTime.Now.ToFileTime().ToString();
            ...
            string documentFile = m_utils.File.Combine(path, "评价项目成本分析表", Word.Tools.DocumentFormat.Normal);
        ...
        private void WriteFirstPage(Costing model)
        {
            ...
            SetCellHeaderText(HYZK.FrameWork.Utility.EnumHelper.GetEnumTitle((EnumType)@Convert.ToInt32(model.Type)).ToString());
