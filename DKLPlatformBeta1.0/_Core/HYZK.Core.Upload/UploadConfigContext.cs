...
namespace HYZK.Core.Upload
{
    public class UploadConfigContext
    {
        ...
        private static string remoteUrl = "localhost";//remote file store path,url
                ...
                if (uploadPath == null)
                {
                    lock (olock)
                    {
                        if (uploadPath == null)
                        {
                            ...
                            {
                                var isLocal = Fetch.ServerDomain.IndexOf(remoteUrl, StringComparison.OrdinalIgnoreCase) < 0;
