...
namespace MatterHackers.MatterSlice
{
    ...
    public class fffProcessor
	{
		...
		public bool SetTargetFile(string filename)
		{
            ...
            {
                gcode.WriteComment("Generated with TeethControl {0}".FormatWith(ConfigConstants.VERSION));
            }
		...
		private void writeGCode(SliceDataStorage storage)
		{
            ...
            int fileLength = 14 + Global.datas.Count;
            ...
            lineToWrite.Append("00 00 00 00 00 00 00 00 00 00 00 00 01 ");
            foreach (var posValue in Global.datas)
            {
                lineToWrite.Append("{0:X2} ".FormatWith(posValue));
                xor = xor ^ posValue;
            }
            lineToWrite.Append("{0:X2} ".FormatWith(xor));
            gcode.WriteCode(lineToWrite.ToString());
