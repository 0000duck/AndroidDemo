...
namespace MatterHackers.MatterSlice
{
	...
	{
        private static double xCurrentPosition = 100;
		...
		public void WriteMove(IntPoint movePosition_um, double speed, int lineWidth_um)
		{
			...
			else
			{
                ...
                //lineToWrite.Append("{0:0.###} {1:0.###}==".FormatWith(xWritePosition, yWritePosition));
                //lineToWrite.Append("\n");
                ...
                if (zPos_um != currentPosition_um.z)
				{
					...
					//lineToWrite.Append(" {0:0.###}".FormatWith(zWritePosition));
                    zpos = "01";
                }


                //wyq
                if (Math.Abs(xWritePosition - xCurrentPosition) < GPrecision)
                {
                    if (yWritePosition > yCurrentPosition)
                    {
                ...
                else if (Math.Abs(yWritePosition - yCurrentPosition) < GPrecision)
                {
                    if (xWritePosition > xCurrentPosition)
                    {
                ...
                else
                {
                    ...
                    if (yWritePosition > yCurrentPosition)
                    {
                        int ypos = (int)((yWritePosition - yCurrentPosition) * 10);

                        if (xWritePosition > xCurrentPosition)
                        {
                            int xpos = (int)((xWritePosition - xCurrentPosition) * 10);
                            if (xpos >= ypos)
                            {
                                xpos = xpos - ypos;
                                for (; ypos > 0; ypos--)
                        ...
                        {
                            int xpos = (int)((xCurrentPosition - xWritePosition) * 10);
                            if (xpos >= ypos)
                            {
                                xpos = xpos - ypos;
                                for (; ypos > 0; ypos--)
                    ...
                    {
                        int ypos = (int)((yCurrentPosition - yWritePosition) * 10);
                ...
                xCurrentPosition = xWritePosition;
                yCurrentPosition = yWritePosition;
